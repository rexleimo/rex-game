  ...
  return (
    <div className="theme-museum site-root">
      ...
      <main className="culture-index">
        <p className="site-hero__kicker">文化馆</p>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.9rem,4vw,2.6rem)', margin: '0.4rem 0 0' }}>
          可读、可核、可玩的民俗导读
        </h1>
        <QuickAnswer
          sentences={[
        ...
        />
        <h2 style={{ fontFamily: 'var(--font-display)', marginTop: '2rem' }}>主题枢纽</h2>
        <div className="culture-index__list">
          {hubs.map((hub) => (
            <Link className="culture-index__card" key={hub.path} href={hub.path}>
              <h2>{hub.h1}</h2>
              <p>{hub.description}</p>
            </Link>
          ))}
        </div>
        <h2 style={{ fontFamily: 'var(--font-display)', marginTop: '2.5rem' }}>专题阅读</h2>
        <div className="culture-index__list">
          {topics.map((topic) => (
            <Link className="culture-index__card" key={topic.path} href={topic.path}>
              <h2>{topic.h1}</h2>
